...
#[cfg(test)]
mod tests {
    use crate::zk_snark::{Bls12Proof, deserialize_proof, deserialize_custom_to_verifying_key};
    use crate::indexed_merkle_tree::{Node, sha256};
    ...
    fn test_validate_epoch_valid_proof() {
        let active_node = Node::initialize_leaf(true, true, Node::EMPTY_HASH.to_string(), Node::EMPTY_HASH.to_string(), Node::TAIL.to_string());
        let inactive_node = Node::initialize_leaf(false, true, Node::EMPTY_HASH.to_string(), Node::EMPTY_HASH.to_string(), Node::TAIL.to_string());
        ...
        let sebastians_node = Node::initialize_leaf(true, true, sebastian, pusch, Node::TAIL.to_string());
        ...
        let result = validate_epoch(&prev_commitment, &current_commitment, proof.clone(), params.vk);
