...
mod tests {
    ...
    async fn setup() -> RedisConnections {
        let redis_connections = RedisConnections::new().unwrap();
        redis_connections.flush_database().await.unwrap();
        redis_connections
    }

    // flush database after each test
    async fn teardown(redis_connections: &RedisConnections) {
        redis_connections.flush_database().await.unwrap();
    }
    ...
    async fn test_get_keys() {
        ...
        let redis_connections = setup().await;
        ...
        let mut keys = redis_connections.get_keys().await.unwrap();
        keys.sort();
    ...
    #[tokio::test]
    async fn test_get_keys_from_empty_dictionary() {
        let redis_connections = setup().await;

        let keys = redis_connections.get_keys().await.unwrap();
    ...
    async fn test_get_too_much_returned_keys() {
        let redis_connections = setup().await;
    ...
    #[tokio::test]
    #[should_panic(expected = "assertion `left == right` failed")]
    async fn test_get_too_little_returned_keys() {
        let redis_connections = setup().await;
    ...
    async fn test_get_hashed_keys() {
        ...
        let keys = redis_connections.get_derived_keys_in_order().await.unwrap();
    ...
    #[tokio::test]
    async fn test_try_getting_hashchain_for_missing_key() {
        let redis_connections = setup().await;
        ...
        let hashchain = redis_connections
            .get_hashchain(&"missing_test_key".to_string())
            .await;
        ...
        teardown(&redis_connections).await;
    }

    #[tokio::test]
    async fn test_try_getting_wrong_formatted_hashchain_value() {
        let redis_connections = setup().await;
        ...
        let hashchain = redis_connections
            .get_hashchain(&"key_to_wrong_formatted_chain_entry".to_string())
            .await;
    ...
    */
    async fn test_update_hashchain() {
        let redis_connections = setup().await;
        ...
        match redis_connections
            .update_hashchain(&incoming_entry, &chain_entries)
            .await
        {
            Ok(_) => (),
