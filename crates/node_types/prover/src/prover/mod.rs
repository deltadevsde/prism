...
pub struct SequencerOptions {
    /// Key used to sign new [`FinalizedEpochs`]. This is only required when the syncer is set up to prove epochs.
...
impl Prover {
    pub fn new(
        ...
        let prover_engine = Arc::new(ProverEngine::new(&opts.prover_engine)?);
